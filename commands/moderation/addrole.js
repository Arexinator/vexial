...
module.exports = {
    ...
    run: async (client, message, args) => {
		...
		if (message.guild.me.roles.highest.comparePositionTo(role2) <= 0) return message.reply({ content: 'The role is above my current role', allowedMentions: {repliedUsers: false} })

        let embed = new MessageEmbed()
        .setDescription(`<a:ec_yes:990478257851858974> ${user} now has the ${role2} role`)
        .setColor("00000")
        let embed2 = new MessageEmbed()
        .setDescription(`<a:ec_yes:990478257851858974> The ${role2} role has been removed from ${user}`)
        .setColor("00000")

        if (message.member.roles.cache.has(role2)) return user.roles.remove(role2).catch(console.log).then(() => message.reply({ embeds: [embed2], allowedMentions: { repliedUser: false }}))
    
		user.roles.add(role2).catch(console.log)
